package org.changgou.content.controller;

import com.alibaba.fastjson.JSON;
import com.github.pagehelper.PageInfo;
import io.swagger.annotations.*;
import org.changgou.content.pojo.Content;
import org.changgou.content.service.ContentService;
...
public class ContentController {
    ...
    public Result refreshCache(@PathVariable(name = "categoryId") Long categoryId) {
        // 先根据category_id查询
        List<Content> byCategoryId = contentService.findByCategoryId(categoryId);
        // 将数据存入redis
        stringRedisTemplate.boundValueOps("content_" + categoryId).set(JSON.toJSONString(byCategoryId));
        return new Result(true, StatusCode.OK, "更新广告缓存数据成功");
    }
